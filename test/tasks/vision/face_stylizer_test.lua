...
local face_stylizer = mediapipe.tasks.lua.vision.face_stylizer
...
local _FaceStylizer = face_stylizer.FaceStylizer
local _FaceStylizerOptions = face_stylizer.FaceStylizerOptions
local _ImageProcessingOptions = image_processing_options_module.ImageProcessingOptions

local _MODEL = 'face_stylizer_color_ink.task'
...
local function test_create_from_file_succeeds_with_valid_model_path(self)
    ...
    local stylizer = _FaceStylizer.create_from_model_path(self.model_path)
...
local function test_create_from_options_succeeds_with_valid_model_path(self)
    ...
    local options = _FaceStylizerOptions(mediapipe_lua.kwargs({ base_options = base_options }))
...
local function test_stylize(self, model_file_type, image_file_name)
    local base_options
...
local function test_stylize_succeeds_with_region_of_interest(self)
    local base_options = _BaseOptions(mediapipe_lua.kwargs({ model_asset_path = self.model_path }))
    local options = _FaceStylizerOptions(mediapipe_lua.kwargs({ base_options = base_options }))
    local stylizer = _FaceStylizer.create_from_options(options)
...
local function test_stylize_succeeds_with_no_face_detected(self)
    local base_options = _BaseOptions(mediapipe_lua.kwargs({ model_asset_path = self.model_path }))
    local options = _FaceStylizerOptions(mediapipe_lua.kwargs({ base_options = base_options }))
    local stylizer = _FaceStylizer.create_from_options(options)
...
describe("FaceStylizerTest", function()
    setUp(_assert)
    ...
    for _, args in ipairs({
        ...
        it("should test_stylize " .. _, function()
            test_stylize(_assert, unpack(args))
        end)
    end

    it("should test_stylize_succeeds_with_region_of_interest", function()
        test_stylize_succeeds_with_region_of_interest(_assert)
    end)

    it("should test_stylize_succeeds_with_no_face_detected", function()
        test_stylize_succeeds_with_no_face_detected(_assert)
    end)
end)
