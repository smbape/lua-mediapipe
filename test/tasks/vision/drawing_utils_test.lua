...
local Connection = mediapipe.tasks.lua.components.containers.landmark.Connection
...
local function test_draw_keypoints_only(self)
    local detection_proto = text_format.Parse([[
        location_data {
            format: BOUNDING_BOX
...
local function test_draw_bboxs_only(self)
    local detection_proto = text_format.Parse([[
        location_data {
            format: BOUNDING_BOX
            bounding_box {xmin: 0 ymin: 0 width: 100 height: 100}
        }
    ]], detection_pb2.Detection())
    local detection = proto_utils.create_detection_from_proto(detection_proto)
    local image = cv2.Mat.zeros(100, 100, cv2.CV_8UC3)
...
local function test_draw_single_landmark_point(self, id, landmark_list_text)
    local landmark_list_proto = text_format.Parse(landmark_list_text,
...
local function test_min_and_max_coordinate_values(self)
    local landmark_list_proto = text_format.Parse([[
