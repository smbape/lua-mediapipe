...
local time_series_header_pb2 = mediapipe.framework.formats.time_series_header_pb2
...
local function test_valid_input_data_type_audio(self)
    local text_config = [[
    ...
    local config_proto = text_format.Parse(
    ...
    local input_audio = _mat_utils.randomImage(1, 16000, cv2.CV_32FC1, 0, 1)
    local results = solution:process({ ['input_audio'] = input_audio })
    self.assertNotEqual(results.output_audio, nil)
    self.assertMatEqual(results.output_audio, input_audio)
end
...
describe("SolutionBaseTest", function()
    ...
    it("should test_valid_input_data_type_audio", function()
        test_valid_input_data_type_audio(_assert)
    end)
