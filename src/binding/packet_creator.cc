...
namespace {
	...
	inline void RaiseErrorIfOverflow(int64_t value, int64_t min, int64_t max) {
		if (value > max) {
			LUA_MODULE_THROW(value << " execeeds the maximum value (" << max << ") the data type can have.");
		}
		else if (value < min) {
			LUA_MODULE_THROW(value << " goes below the minimum value (" << min << ") the data type can have.");
		}
	}
}
...
namespace mediapipe::lua::packet_creator {
	...
	std::shared_ptr<Packet> create_image_frame(const ImageFrame& data, ImageFormat::Format image_format, bool copy) {
		LUA_MODULE_ASSERT_THROW(data.Format() == image_format, "The provided image_format doesn't match the one from the data arg.");
		LUA_MODULE_ASSERT_THROW(copy, "Creating ImageFrame packet by taking a reference of another ImageFrame object is not supported yet.");
	...
	std::shared_ptr<Packet> create_image(const Image& image, ImageFormat::Format image_format, bool copy) {
		LUA_MODULE_ASSERT_THROW(image.image_format() == image_format, "The provided image_format doesn't match the one from the data arg.");
		LUA_MODULE_ASSERT_THROW(copy, "Creating Image packet by taking a reference of another Image object is not supported yet.");
	...
	std::shared_ptr<Packet> create_image(const cv::Mat& data, bool copy) {
		if (!copy) {
			LUA_MODULE_WARN("'data' is still writeable. Taking a reference of the data to create Image packet is dangerous.");
		}
		MP_ASSIGN_OR_THROW(auto image_frame, CreateImageFrame(data, copy));
		return CreateImagePacket(image_frame);
	}
	...
	std::shared_ptr<Packet> create_matrix(const cv::Mat& data, bool transpose) {
		...
		LUA_MODULE_ASSERT_THROW(data.type() == cv::traits::Type<_Tp>::value, "The data should be a float matrix");
		LUA_MODULE_ASSERT_THROW(data.dims <= 2, "The data is expected to have at most 2 dimensions");
		LUA_MODULE_ASSERT_THROW(data.cols == 1 || data.channels() == 1, "The data is expected be a 2d matrix");
	...
	std::shared_ptr<Packet> create_proto(const google::protobuf::Message& message) {
		...
		LUA_MODULE_ASSERT_THROW(message.SerializeToString(&serialized), "Failed to serialize message " << type_name);

		absl::StatusOr<Packet> packet_status = packet_internal::PacketFromDynamicProto(type_name, serialized);
		LUA_MODULE_ASSERT_THROW(packet_status.ok(), "Unregistered proto message type: " << type_name);
