...
namespace {
	...
	using namespace mediapipe::tasks::lua::vision::face_detector;
...
namespace mediapipe::tasks::lua::vision::face_detector {
	...
	absl::StatusOr<std::shared_ptr<FaceDetector>> FaceDetector::create(
		...
		return BaseVisionTaskApi::create(L, graph_config, running_mode, std::move(packet_callback), static_cast<FaceDetector*>(nullptr));
	}

	absl::StatusOr<std::shared_ptr<FaceDetector>> FaceDetector::create_from_model_path(lua_State* L, const std::string& model_path) {
		auto base_options = std::make_shared<BaseOptions>(model_path);
		return create_from_options(L, std::make_shared<FaceDetectorOptions>(base_options, VisionTaskRunningMode::IMAGE));
	}

	absl::StatusOr<std::shared_ptr<FaceDetector>> FaceDetector::create_from_options(lua_State* L, std::shared_ptr<FaceDetectorOptions> options) {
