...
namespace {
	...
	using namespace mediapipe::tasks::lua::vision::core::base_vision_task_api;
	...
	using namespace mediapipe::tasks::lua::vision::image_segmenter;
...
namespace mediapipe::tasks::lua::vision::image_segmenter {
	...
	absl::StatusOr<std::shared_ptr<ImageSegmenter>> ImageSegmenter::create(
		...
		MP_ASSIGN_OR_RETURN(auto vision_task_api, BaseVisionTaskApi::create(L, graph_config, running_mode, std::move(packet_callback), static_cast<ImageSegmenter*>(nullptr)));
	...
	ImageSegmenter::ImageSegmenter(
		...
		VisionTaskRunningMode running_mode
	) : BaseVisionTaskApi(L, runner, running_mode) {
	...
	absl::StatusOr<std::shared_ptr<ImageSegmenter>> ImageSegmenter::create_from_model_path(lua_State* L, const std::string& model_path) {
		auto base_options = std::make_shared<BaseOptions>(model_path);
		return create_from_options(L, std::make_shared<ImageSegmenterOptions>(base_options, VisionTaskRunningMode::IMAGE));
	}

	absl::StatusOr<std::shared_ptr<ImageSegmenter>> ImageSegmenter::create_from_options(lua_State* L, std::shared_ptr<ImageSegmenterOptions> options) {
