...
namespace {
	...
	using namespace mediapipe::tasks::lua::vision::core::image_processing_options;
	using namespace mediapipe::tasks::lua::vision::core::vision_task_running_mode;
	using namespace mediapipe::tasks::lua::vision::object_detector;
	using namespace mediapipe::tasks::vision::object_detector::proto;

	using mediapipe::lua::PacketsCallback;
	using mediapipe::tasks::core::PacketMap;

	using Detection = mediapipe::tasks::lua::components::containers::detections::Detection;
...
namespace mediapipe::tasks::lua::vision::object_detector {
	absl::StatusOr<std::shared_ptr<mediapipe::tasks::vision::object_detector::proto::ObjectDetectorOptions>> ObjectDetectorOptions::to_pb2() const {
		...
		if (!category_allowlist.empty()) pb2_obj->mutable_category_allowlist()->Add(category_allowlist.begin(), category_allowlist.end());
		if (!category_denylist.empty()) pb2_obj->mutable_category_denylist()->Add(category_denylist.begin(), category_denylist.end());
	...
	absl::StatusOr<std::shared_ptr<ObjectDetector>> ObjectDetector::create(
		...
		return BaseVisionTaskApi::create(L, graph_config, running_mode, std::move(packet_callback), static_cast<ObjectDetector*>(nullptr));
	}

	absl::StatusOr<std::shared_ptr<ObjectDetector>> ObjectDetector::create_from_model_path(lua_State* L, const std::string& model_path) {
		auto base_options = std::make_shared<BaseOptions>(model_path);
		return create_from_options(L, std::make_shared<ObjectDetectorOptions>(base_options, VisionTaskRunningMode::IMAGE));
	}

	absl::StatusOr<std::shared_ptr<ObjectDetector>> ObjectDetector::create_from_options(lua_State* L, std::shared_ptr<ObjectDetectorOptions> options) {
