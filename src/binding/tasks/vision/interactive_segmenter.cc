...
#include "binding/tasks/vision/interactive_segmenter.h"
...
namespace {
	...
	using namespace mediapipe::tasks::lua::vision::interactive_segmenter;
	...
	[[nodiscard]] absl::StatusOr<std::shared_ptr<RenderData>> _convert_roi_to_render_data(const RegionOfInterest& roi) {
		...
		MP_ASSERT_RETURN_IF_ERROR(roi.format != RegionOfInterest::Format::UNSPECIFIED, "RegionOfInterest format not specified.");
		MP_ASSERT_RETURN_IF_ERROR(roi.format == RegionOfInterest::Format::KEYPOINT, "Unrecognized format.");
