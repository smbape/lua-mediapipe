...
namespace {
	...
	using namespace mediapipe::tasks::lua::vision::holistic_landmarker;
...
namespace mediapipe::tasks::lua::vision::holistic_landmarker {
	...
	absl::StatusOr<std::shared_ptr<HolisticLandmarker>> HolisticLandmarker::create(
		...
		return BaseVisionTaskApi::create(L, graph_config, running_mode, std::move(packet_callback), static_cast<HolisticLandmarker*>(nullptr));
	}

	absl::StatusOr<std::shared_ptr<HolisticLandmarker>> HolisticLandmarker::create_from_model_path(lua_State* L, const std::string& model_path) {
		auto base_options = std::make_shared<BaseOptions>(model_path);
		return create_from_options(L, std::make_shared<HolisticLandmarkerOptions>(base_options, VisionTaskRunningMode::IMAGE));
	}

	absl::StatusOr<std::shared_ptr<HolisticLandmarker>> HolisticLandmarker::create_from_options(lua_State* L, std::shared_ptr<HolisticLandmarkerOptions> options) {
