...
namespace mediapipe::tasks::lua::vision::gesture_recognizer {
	...
	public:
		...
		CV_WRAP [[nodiscard]] static absl::StatusOr<std::shared_ptr<GestureRecognizer>> create_from_model_path(lua_State* L, const std::string& model_path);
		CV_WRAP [[nodiscard]] static absl::StatusOr<std::shared_ptr<GestureRecognizer>> create_from_options(lua_State* L, std::shared_ptr<GestureRecognizerOptions> options);
