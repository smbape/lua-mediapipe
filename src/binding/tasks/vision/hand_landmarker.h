...
namespace mediapipe::tasks::lua::vision::hand_landmarker {
	...
	public:
		...
		CV_WRAP [[nodiscard]] static absl::StatusOr<std::shared_ptr<HandLandmarker>> create_from_model_path(lua_State* L, const std::string& model_path);
		CV_WRAP [[nodiscard]] static absl::StatusOr<std::shared_ptr<HandLandmarker>> create_from_options(lua_State* L, std::shared_ptr<HandLandmarkerOptions> options);
