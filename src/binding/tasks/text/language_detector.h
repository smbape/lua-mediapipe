...
namespace mediapipe::tasks::lua::text::language_detector {
	...
	struct CV_EXPORTS_W_SIMPLE LanguageDetectorResult {
		...
		CV_WRAP LanguageDetectorResult(
			const std::vector<std::shared_ptr<LanguageDetectorResult::Detection>>& detections = std::vector<std::shared_ptr<LanguageDetectorResult::Detection>>()
		...
		CV_PROP_RW std::vector<std::shared_ptr<LanguageDetectorResult::Detection>> detections;
	};
	...
	class CV_EXPORTS_W LanguageDetector : public ::mediapipe::tasks::lua::text::core::base_text_task_api::BaseTextTaskApi {
	public:
