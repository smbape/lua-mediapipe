...
module.exports = ({language}) => {
    ...
    return [
        ...
        [`struct ${ nsProgId }.PoseLandmarkerResult`, "", [`/progid=${ progid }.PoseLandmarkerResult`, "/Simple", "/DC"], [
            ...
            ["std::vector<std::vector<components::containers::NormalizedLandmark>>", "pose_landmarks", "", ["/RW", `/RExpr=${ containers }::CppConvertToNormalizedLandmarkListList($0)`, `/WExpr=${ containers }::CppConvertToNormalizedLandmarksList($value, $0)`]],
            ["std::vector<std::vector<components::containers::Landmark>>", "pose_world_landmarks", "", ["/RW", `/RExpr=${ containers }::CppConvertToLandmarkListList($0)`, `/WExpr=${ containers }::CppConvertToLandmarksList($value, $0)`]],
        ], "", ""],
    ];
};
