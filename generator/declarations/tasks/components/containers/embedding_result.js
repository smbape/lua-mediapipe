...
module.exports = ({language, self_get}) => {
    ...
    const setterEmbedding = `
        if ($value->depth() == CV_32F) {
            ...
            return luaL_error(L, "depth must be one of CV_8U, CV_32F");
        }
    `.replace(/^ {8}/mg, "").trim();
    ...
    return [
        ...
        [`struct ${ nsProgId }.Embedding`, "", [`/progid=${ progid }.Embedding`, "/Simple"], [
            ...
            ["std::string", "quantized_embedding", "", ["/RW", "/WType=std::variant<std::string, std::vector<unsigned char>>", `/WExpr=
        ...
        [`${ nsProgId }.Embedding.Embedding`, "", ["/Expr=", `/DC=
            ...
            else { return luaL_error(L, "depth must be one of CV_8U, CV_32F"); }
