...
class Parser {
        ...
        for (const [proto, raw_fields] of this.exports.entries()) {
            ...
            for (const [field_type, field_name, field_rule] of raw_fields.fields) {
                ...
                if (field_rule === "repeated") {
                    ...
                    body.push(`
                        if (${ field_name }_positional || ${ field_name }_kwarg) {
                            MP_RETURN_LUA_ERROR_IF_ERROR(${ cmessage }::CopyFrom(shared_message->mutable_${ field_name }(), ${ field_name }.get()));
                        }
                    `.trim().replace(/^ {24}/mg, ""));

                    fields.push([argtype, field_name, "", [
                        `/R=mutable_${ field_name }`,
                        `/WExpr=MP_RETURN_LUA_ERROR_IF_ERROR(${ cmessage }::CopyFrom(${ self_get(`mutable_${ field_name }()`) }, $value.get()))`,
                    ]]);
                }
            ...
            for (let i = 0; i < Math.min(1, parts.length); i++) {
    ...
    addRepeatedField(proto, fields, field_type, field_name, scopes, body, i) {
        ...
        fields.push([AnyObject, field_name, "", [
            `/R=mutable_${ field_name }`,
            `/WExpr=
                auto shared_message = reference_internal(${ self });
                ${ setter.split("\n").join(`\n${ " ".repeat(16) }`) }
            `.replace(/^ {16}/mg, "").trim()
        ]]);
        ...
        const ssize_t = `::google::protobuf::${ language }::ssize_t`;
        ...
        decls.push(...[
            ...
            [`${ fqn }.table`, "void", ["/Call=lua_push", `/Expr=L, ${ self }`], [], "", ""],

            [`${ fqn }.size`, "int", ["=sol::meta_function::length"], [], "", ""],

            [`${ fqn }.${ byref ? "Mutable" : "Get" }`, `${ value_type }${ byref }`, ["=sol::meta_function::index"], [
                ["int", "index", "", []],
            ], "", ""],
        ]);
